...
contract DualHandler is Test {
    ...
    constructor(VulnNFT _vulnNft, FixedNFT _fixedNft, VulnerableBROVault _vuln, FixedBROVault _fixedVault) {
        vulnNft = _vulnNft;
        fixedNft = _fixedNft;
        vuln = _vuln;
    ...
    function fixedTotal() public view returns (uint256) {
        return fixedVault.broBalance(user1) + fixedVault.broBalance(user2) + fixedVault.broBalance(attacker);
    }

    function vulnTotal() public view returns (uint256) {
        return vuln.broBalance(user1) + vuln.broBalance(user2) + vuln.broBalance(attacker);
    }
}
...
contract BROVaultDualInvariantTest is StdInvariant, Test {
    ...
    function invariant_fixedMatchesExpected() public view {
        assertEq(handler.fixedTotal(), handler.fixedExpected(), "fixed vault accounting drifted");
    }

    function invariant_vulnerableNotLessThanFixed() public view {
        assertGe(handler.vulnTotal(), handler.fixedTotal(), "vulnerable vault should not under-credit vs fixed");
    }
}
